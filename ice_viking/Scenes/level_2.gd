extends Node2D  # Skripta za upravljanje drugim levelom

@onready var player = get_tree().get_first_node_in_group("Player")  # Dohvaćam igrača iz scene

func _ready():
	# Postavljam osnovne podatke o drugom levelu i coinima na početku igre
	Global.current_level = "res://Scenes/Level_2.tscn"
